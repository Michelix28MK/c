...
int main (void)
{
    ...
    int trova_re(int tavola[][DIM], int funzione);//controlla la presenza di entrambi i re
...
void show_tavola(int tavola[][DIM])
...
int next_move(int tavola[][DIM], int *numero_giocatore)
{
    ...
    int move_torre(int tavola[][DIM], int posY, int posX, int *utente, int pos1Y, int pos1X, int regina);

    void move_cavallo(int tavola[][DIM], int posY, int posX, int *utente, int pos1Y, int pos1X);

    int move_alfiere(int tavola[][DIM], int posY, int posX, int *utente, int pos1Y, int pos1X, int regina);
    ...
    { 
        do
        {
            ...
            for (int i = 0; i < sizeof(lettera); i++) //trovo l'indice della lettera
                for (int j = 0; j < sizeof(lettere_tavola); j++) 
    ...
    case 'A':
        printf("\n\nspostamento alfiere ");
        move_alfiere(tavola, numeri[0]-1, numeri[2], numero_giocatore, numeri[1]-1, numeri[3], 0);
        break;
    case 'q':
    case 'Q':
        printf("\n\nspostamento regina ");
...
int move_pedone(int tavola[][DIM], int posY, int posX, int *utente, int pos1Y, int pos1X)
{
    ...
    */
   int primo_spostamento_Y, spostamento_Y_doppio_valido, spostamento_Y_singolo_valido, spostamento_X_valido, spostamento_X_nullo, cella_frontale_libera, due_cella_frontali_libere, nemico_nella_cella_DX, nemico_nella_cella_SX, fine_tavola, exit = 0;
   char pedina;

    if (*utente)
    {       // pedine a disposizione dalla 17 alla 32
    ...
    }else{ //pedine a disposizione dalla 1 alla 16 alla 
    ...
    if (fine_tavola) //se il pedone selezionato riesce ad arrivare alla fine allora gli si fascegliere una figura con cui sostituirlo
    {
        do
        {
            ...
            if (!*utente)
            {
                ...
                case 't':
                    printf("\n\ntorre ");
                    tavola[pos1Y][pos1X] = 17;
                    break;
                case 'c':
                    printf("\n\nsostituzione in alfiere ");
                    tavola[pos1Y][pos1X] = 18;
                    break;
                case 'a':
                    printf("\n\nsostituzione in alfiere ");
                    tavola[pos1Y][pos1X] = 19;
                    break;
                case 'q':
                    printf("\n\nsostituzione in regina ");
                    tavola[pos1Y][pos1X] = 21;
                    break;
                default:
                    printf("Error: non hai selezionato una pedina!");
                    exit = 1;
...
int move_torre(int tavola[][DIM], int posY, int posX, int *utente, int pos1Y, int pos1X, int regina)
{
   ...
   int exit = 0;
   int movimento_nullo = ((pos1X-posX) == 0) && ((pos1Y-posY) == 0);
   int movimento_1 = ((pos1X-posX) == 0) && ((pos1Y-posY) != 0);
   int movimento_2 = ((pos1X-posX) != 0) && ((pos1Y-posY) == 0);
   int libera_amica;
   int libero_percorso = 0;
...
{
    int movimento_nullo = ((pos1X-posX) == 0) && ((pos1Y-posY) == 0);
    int movimento_V = ((pos1Y-posY) == 2 ||  (pos1Y-posY) == -2) && ((pos1X-posX) == 1 ||  (pos1X-posX) == -1);
    int movimento_O = ((pos1Y-posY) == 1 ||  (pos1Y-posY) == -1) && ((pos1X-posX) == 2 ||  (pos1X-posX) == -2);
    int libera_amica;
...
int move_alfiere(int tavola[][DIM], int posY, int posX, int *utente, int pos1Y, int pos1X, int regina)
{
    int exit = 0;
    int movimento_nullo = ((pos1X-posX) == 0) && ((pos1Y-posY) == 0);
    int movimento_valido = (((pos1X-posX) == (pos1Y-posY)) || ((pos1X-posX) == (-1*(pos1Y-posY)))); //sicuramente diagonale
    int libera_amica;
    int libero_percorso = 0;
